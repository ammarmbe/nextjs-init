...
class ProjectInitializer {
  ...
  async initiateORM() {
    await this.copy("src/ORM/.env.example", ".env");

    switch (this.ORM) {
      case "prisma":
        await this.copy("src/ORM/prisma.db.ts", "src/utils/db.ts");
        ...
        break;
      case "neon":
        await this.copy("src/ORM/neon.db.ts", "src/utils/db.ts");
  ...
  async initiateData() {
    switch (this.data) {
      case "client":
        ...
        await this.copy("src/data/client/trpc", "src/trpc");
        await this.copy("src/data/client/server", "src/server");

        await this.del("src/app/layout.tsx");
        await this.copy("src/data/client/layout.tsx", "src/app/layout.tsx");

        if (this.auth) {
          await this.copy("src/data/client/auth.trpc.ts", "src/server/trpc.ts");
        } else {
          await this.copy("src/data/client/trpc.ts", "src/server/trpc.ts");
        }

        break;
      case "server":
        await this.copy("src/data/server/data", "src/data");
  ...
  async initiateAuth() {
    ...
    switch (this.auth) {
      case "username":
        ...
        await this.copy("src/auth/username/login", "src/app/login");
        await this.copy("src/auth/username/schemas", "src/schemas");
      ...
      case "google":
        ...
        await this.copy("src/auth/google/api", "src/app/api");
